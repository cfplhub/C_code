...
void student::applyOrder()
{
	...
	//cout << "4号机房容量：" << vCom[3].m_MaxNum << endl;
...
void student::showMyOrder()
{
	...
	for (int i = 0; i < of.m_Size; i++)
	{
		...
		{
			cout << "预约时间:  周" << of.orderDate[i]["date"];
			cout << "  预约时段:" << (of.orderDate[i]["interval"] == "1" ? "上午" : "下午");
			cout << "  机房:" << of.orderDate[i]["roomId"];
			string status = " 状态:";
...
void student::showAllOrder()
{
	...
	for (int i = 0; i < of.m_Size; i++)
	{
		...
		cout << "  预约时段:" << (of.orderDate[i]["interval"] == "1" ? "上午" : "下午");
		cout << "  学号：" << of.orderDate[i]["stuId"];
		cout << "  姓名：" << of.orderDate[i]["stuName"];
		cout << "  机房编号:" << of.orderDate[i]["roomId"];
		string status = " 状态:";
		if (of.orderDate[i]["status"] == "1")
		{
			status += "审核中";
	...
	for (int i = 0; i < of.m_Size; i++)
	{
		if (atoi(of.orderDate[i]["stuId"].c_str()) == this->m_Id )
		{
			if (of.orderDate[i]["status"] == "1" || of.orderDate[i]["status"] == "2")
			{
				...
				cout << index++ << "、";
				cout << "预约日期： 周" << of.orderDate[i]["date"];
				cout << " 时间段：" << (of.orderDate[i]["interval"] == "1" ? "上午" : "下午");
				cout << " 机房编号：" << of.orderDate[i]["roomId"];
				string status = " 状态:";
				if (of.orderDate[i]["status"] == "1")
				{
